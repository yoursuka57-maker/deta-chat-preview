      ...
      {!isCollapsed && (
        <div className="flex items-center px-4 py-4 gap-3">
          ...
          <Button
            ...
            className="ml-auto h-7 w-7 rounded-full border border-sidebar-border bg-sidebar-background hover:bg-sidebar-accent flex items-center justify-center p-0"
          >
            <PanelLeft className="h-4 w-4 rotate-0 transition-transform duration-200" />
          </Button>
        </div>
      )}

      {/* Navigation */}
      <nav className="flex-1 px-2 space-y-2 overflow-hidden flex flex-col relative">
        {/* Collapse Button when closed - above New Chat */}
        {isCollapsed && (
          <div className="flex justify-center pt-4">
            ...
            >
              <PanelLeft className="h-4 w-4 rotate-180 transition-transform duration-200" />
            </Button>
          </div>
        )}
        ...
        <Button
          variant="ghost"
          onClick={onNewChat}
          className={`w-full ${isCollapsed ? "justify-center" : "justify-start"} gap-3 hover:bg-sidebar-accent hover:text-primary transition-smooth mt-4`}
          title="New Chat"
        >
